...
setwd("~/Corsi/Statistica Applicata/Applied Statistics MATE 21-22/Lab 11 - 13052022")
...
sapply(c(beta0,beta1,beta2),as.numeric)#c(beta0=as.numeric(beta0),beta1=as.numeric(beta1),beta2=as.numeric(beta2))
fm$coefficients
...
yhat.lm <- cbind(rep(1,n), speed1, speed2)%*%coef(fm) # ripetizione?
...
select(fit.ridge) # note that the model with the optimal lambda has already been computed!

# or
lambda.opt <- lambda.c[which.min(fit.ridge$GCV)] # thanks to having fitted with a sequence of lambdas
lambda.opt
...
plot(cv.ridge)
abline(v=log(bestlam.ridge), lty=1) # soldi: optimal lambda # dashed: biggest not to far from the optimal one (1 std from the optimal)
