...
## Ex. 1 (2 classes, univariate)

### LDA

```{r }
cyto <- read.table(here::here('markdowns','lab_8_data','cytokines.txt'), header=T)
cyto
...
colors <- c('blue','red')
plot(cyto[,1], cyto[,2], pch=19, col=colors[group], xlab='Inf-g', ylab='IL-5')

```

Idea: we aim to find a *rule* to classify patients as Group `A` or Group `B` given the measurements of `Infg` (Interferon gamma) and `IL5` (Interleukin 5).
In fact, we only consider the variable `Infg` since there is no statistical evidence to state that there is a difference in mean along the component `IL5`.

**Exercise**: build a confidence region of level 95% for the difference of the means between the two groups (independent populations) and verify the previous statement.
...
Don't reject normality.

```{r}
# One-sample t-test
res <- t.test(diff, mu = mu0, conf.level = 1-alpha, alternative = 'two.sided')

res$p.value
```

We don't reject the Null Hypothesis, i.e. we don't reject that the mean of `diff` is 0, i.e. we don't reject that the two populations have the same mean.
...
Recall: the classification region is obtained by comparing $f_A p_A$ with $f_B p_B$.
...
x <- seq(range(Infg)[1], range(Infg)[2], 0.5) # include the range of Infg
...
#### Posterior probability and classification
...
- in lda: the coordinates of the canonical analysis of Fisher (Fisher's discriminant scores)
...
plot(Infg[A], rep(0, length(A)),
     pch=16, col='blue', ylim=c(0,1), xlim=range(Infg),
     xlab='x', ylab='estimated posterior', main="LDA")
points(Infg[B], rep(0, length(B)), pch=16, col='red')
abline(v=0, col='grey')
points(c(0,0),c(predict(cyto.lda, x)$posterior),
...
Let's change `k` (when `k=12` you are actually looking at all the neighbors)
...
We consider only the first two variables, `Sepal.Length` and `Sepal.Width` (`p=2`, `g=3`); we aim to build a classifier based on the characteristic of the sepal that identifies the iris species.
...
```{r }
set.seed(1)
iris2 <- iris2 + cbind(rnorm(n, sd=0.025))
```
...
- `Coefficients of linear discriminants`: versors of the canonical directions (to be read column-wise)
- `Proportion of trace`: proportion of variance explained by the corresponding canonical direction

Estimate of **AER (actual error rate)**.

1. APER (apparent error rate, i.e. on the training set)
2. Compute the estimate of the AER by leave-one-out cross-validation

1. APER

Use the classifier on the training set
...
```{r }
table(class.true = species.name,
      class.assigned = Lda.iris$class)
...
Remark: this is correct only if we estimate the prior with the empirical frequencies (as it's done by default)! Otherwise:

```{r eval=FALSE}
prior <- c(1/3,1/3,1/3)
G <- 3
misc <- table(class.true = species.name,
              class.assigned = Lda.iris$class)
APER <- 0
for(g in 1:G)
...
The vending machines of the Exxon fuel contain optical detectors able to measure the size of the banknotes inserted. Knowing that 0.1% of the 10\$ banknotes in circulation are counterfeit, Exxon would like to implement a software to identify false 10\$ banknotes, as to minimize the economic losses.
...
a. build an appropriate classifier, estimating the unknown parameters starting from the two datasets `moneytrue.txt` and `moneyfalse.txt`, containing about 100 true banknotes and 100 counterfeit banknotes (in mm). Qualitatively show the two classification regions in a graph;
...
```{r }
true <- read.table(here::here('markdowns','lab_8_data','moneytrue.txt'),header=TRUE)
false <- read.table(here::here('markdowns','lab_8_data','moneyfalse.txt'),header=TRUE)
...
```{r }
c.vf <- 10 # cost of classifying as true something false, very high
c.fv <- 0.05 # cost of classifying as false something true, very low
```

Prior probabilities (problem-domain knowledge)
...
```{r }
prior.c <- c(pt*c.fv, # lower prior since it costs less to misclassify it
             pf*c.vf) # higher prior since it costs more to misclassify it
prior.c <- prior.c/(c.vf*pf+c.fv*pt)
prior.c
```
...
$$
\frac{(\text{prob of true})\cdot(\text{error made})}{(\text{everything under true})} + \frac{(\text{prob of false})\cdot(\text{error made})}{(\text{everything under false})} = \frac{(\text{prob of true})\cdot 2 }{(2+98)} + \frac{(\text{prob of false})\cdot 80 }{(80+20)}
$$

The model is not good, it commits a lot of errors in the training set (88 falses classified as true!), however, we have to correct for the real case. Since in reality there aren't many fake banknotes, the APER is still quite low.
...
This is similar to the above, we calculate the probability of committing an error and multiply by how much we pay for it. In the first term, we have `pt` probability of being true, in such case we misclassify `2` out of `98+2=100` and we pay `c.fv` for that.
...
Let's change viewpoint: we look for the directions that highlight the discrimination among groups -> we look for the canonical directions.

Assumptions: homogeneity of the covariance structure (we relax the normality assumption). Let's consider again the `iris` dataset
...
svmfit <- svm(y~., data=dat , kernel ='linear',
              cost = 10, scale = FALSE) # data is not normalized
summary(svmfit)
```

7 support vectors (4 of the first class, 3 of the second class) have been found.
...
Support vectors are indicated with crosses. They are:
...
If we try to change the cost parameter we get more support points (higher bias, lower variance)
...
To set the parameter `cost` we can use the function `tune()`, which is based on cross-validation (10-fold)
...
If the classes are separable, setting a high value for the cost function leads to the maximal margin classifier (i.e., it returns the classification provided by the best separating hyperplane).
...
```{r }
plot(x, col = ifelse(y==1, 'salmon', 'light blue'),
     pch=19, xlab='x1', ylab='x2', asp=1)
```
...
svmfit <- svm(y~., data=dat[train ,], kernel = 'radial',
              gamma = 1, cost = 1)
summary(svmfit)
plot(svmfit, data = dat, col = c('salmon', 'light blue'), pch=19, asp=1)
```
...
Increasing the cost decreases the errors on the training set, at the expense of a more irregular boundary
