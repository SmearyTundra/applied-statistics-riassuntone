...
attach(data)
walk <- which(data$activity == "walking")
sit <- which(data$activity == "sitting")
lay <- which(data$activity == "laying")
...
Ss
Sl
bartlett.test(accel ~ data$activity)
bartlett.test(gyro ~ data$activity)
...
t <- table(true = data$activity, classif = q.APER.class)
gr <- dim(t)[1]
APER <- 0
for(i in 1:gr){
...
library(class)
kn_5 <- knn(train = data[,1:2], test = ag, cl = data$activity, k = 5, prob=T)

z_k  <- as.numeric(kn_5)

x11()
plot(data[,1:2], main = "Classification regions k = 5")
points(data[c(walk),1:2], col = 'red',pch=19)
points(data[sit,1:2], col = 'blue')
points(data[lay,1:2], col = 'green')
