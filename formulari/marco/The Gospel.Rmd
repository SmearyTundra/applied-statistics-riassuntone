...
This is an R Markdown document. It summarizes the lab session of the course of Applied Statistics
held by professor Secchi. We remember that Grasselli's mercy, grace and favour are made available
for us through the intercession of Secchi.
...
for(level in levels(combined)){
  Ps_combined = c(Ps_combined, mcshapiro.test(as.data.frame(data2ANOVA[combined == level,id_categories]))$p) # look for big p-value 
}
...
semi_ax = r*sqrt(eigen(df_cov)$values)  
...
head(df)
plot(df$t, df$h)
mod = lm(h ~ I(sin(2*pi*t/28)) + I(sin(pi*(t-82)/365)) + t, data = df)
summary(mod)
...
g.t <- gstat(formula = log.sights ~ log.chlorofill , data = df, model = v.fit1)

# predict hotels[1,]
# Estimate the mean: use the argument 'BLUE=TRUE' otherwise the observation
y1 <- predict(g.t, df[1,], BLUE = TRUE)$var1.pred
y2 <- predict(g.t, df[2,], BLUE = TRUE)$var1.pred

# Coefficients
a1 = (y2-y1)/(df[2,]$log.chlorofill - df[1,]$log.chlorofill)
a0 = y1 - a1*df[1,]$log.chlorofill
...
data_fd_1 = Data2fd(y = data,argvals = time,basisobj = basis_1)
plot.fd(data_fd_1)
...
x_max = dim(data)[1]
time = 1:24
basis = create.bspline.basis(rangeval=c(0,x_max),nbasis=n_bas, norder=norder)
